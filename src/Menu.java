...
class Menu extends GUI_Management{
    ...
    private void basicSpotInfo(JPanel panel, Spot a) {
        ...
        person = super.event.d.people.get(indexUser);
        if(person.chosenSpots.contains(a)){
    ...
    private void addActionButton(JButton add, Spot a, JLabel subL, String instBar){
        Person person;
        person = super.event.d.people.get(indexUser);
        int subs = person.chosenSpots.size();
        String extraWarning = "";
        ...
        if (Objects.equals(add.getText(), "Subscribe to")) {
            if (subs >= 5) {
                ...
                if (a instanceof Bar) {
                    ...
                    if (max_guest > size_guest){
                        ((Bar) a).getGuestList().add(person);
                    } else if (person.profile.equals("Bohemian")) {
                        int i;
                        int svd = -1;
            ...
            super.event.d.revenue += a.getCost(person);
        } else {
            add.setText("Subscribe to");
            person.chosenSpots.remove(a);
            a.subs--;
            if(a instanceof Bar){
                if(((Bar) a).getGuestList().contains(person)){
                    int nextGuest;
                    ((Bar) a).getGuestList().remove(person);
                    ...
                    ((Bar) a).getWaiting().remove(person);
                }
            }
            super.event.d.revenue -= a.getCost(person);
        }

        subs = person.chosenSpots.size();
